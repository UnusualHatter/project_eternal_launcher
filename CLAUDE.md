...
- On first run, or when the file is still on the canonical 1920×1080@60 defaults, calls `DisplayDetectionService.GetPrimaryDisplay()` (Win32 `EnumDisplaySettings`) to seed `Width` / `Height` / `RefreshRate` from the user's actual primary monitor.
...
`SettingsViewModel` works with a `SettingsModel` instance and binds bind-editing, display mode, validation state, and the schema-driven category list (see below).
...
- `Cvar` — the TF2 cvar name (used in tooltips + autoexec lines).
- `CustomEmitter` — optional callback that returns the lines to write when the default `cvar 0/1` / `cvar value` format isn't right (e.g. `mat_disable_bloom` inverts, `closecaption` emits two lines).
- `NotCasualCompatible` — true for settings whose cvar is blocked on sv_pure (TF2 Casual / community pub servers). The UI renders a small "not casual compatible" chip next to the title.
- `DependsOn` + `IsEnabledPredicate` — wire a child row to a parent toggle (e.g. *Medic autocall threshold* is enabled only while *Medic autocall* is on). When the parent flips, child wrappers re-raise `IsEnabled`; the XAML row dims (Opacity 0.4) and disables input.
...
`AutoexecWriter.WriteToAutoexec` iterates `SettingsSchema.Build(settings)`, calls `SettingItem.EmitCvarLines()` on each entry, and groups the output by category header (`// --- Gameplay ---`, etc.). Items that return nothing are skipped, so the cfg never picks up junk lines for off-toggles or default values. The managed block sits between `// === ETERNAL LAUNCHER MANAGED BLOCK …` markers; everything outside that block is treated as user-owned content and preserved verbatim across writes.

`AutoexecParser.LoadFromAutoexec` runs at startup against the *entire* file (managed + user-owned) — installing the launcher onto an existing TF2 install with a hand-written autoexec automatically lifts those values into the UI on first run. The parser mirrors every `CustomEmitter` in the schema, including inversions like `mat_disable_bloom 1 → Bloom = false` and `cl_jiggle_bone_framerate_cutoff 0 → DisableJiggleBones = true`.

### Sidebar scroll anchors

The Settings sidebar uses an attached property (`Core/ScrollAnchor.cs`) that registers each section's element under its anchor name (`general`, `gameplay`, `competitive`, …) at `Loaded` time. Sidebar clicks raise `SettingsViewModel.ScrollToCategoryRequested`, which the code-behind resolves through the same registry. Categories generated by `DataTemplate` (where `x:Name` is unreachable) are addressable because the attached property is set on the template root. `ContentScroller.ScrollChanged` walks the registry every scroll tick and updates `ActiveSidebarId`, which the sidebar entries `DataTrigger`-bind for the active-highlight.
